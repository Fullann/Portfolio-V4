...
async function insertDefaultData() {
  try {
    ...
    const [personalInfoRows] = await pool.execute(
...
const dbOperations = {
  ...
  portfolioProjects: {
    ...
    update: async (id, data) => {
      ...
      if (data.repoLink !== undefined) {
        fields.push("repo_link = ?");
        values.push(data.repoLink);
      }
      if (data.liveLink !== undefined) {
        fields.push("live_link = ?");
        values.push(data.liveLink);
      }
      if (data.filterCategory !== undefined) {
        fields.push("filter_category = ?");
        values.push(data.filterCategory);
      }
  ...
  blogs: {
    ...
    delete: async (id) => {
      const [result] = await pool.execute("DELETE FROM blogs WHERE id = ?", [
        id,
      ]);
      return result;
    },
  ...
  personalInfo: {
    ...
    update: async (data) => {
      ...
      if (data.aboutText !== undefined) {
        fields.push("about_text = ?");
        values.push(data.aboutText);
      }
      if (data.cvFile !== undefined) {
        fields.push("cv_file = ?");
        values.push(data.cvFile);
      }
  ...
  skills: {
    ...
    getById: async (id) => {
      const [rows] = await pool.execute("SELECT * FROM skills WHERE id = ?", [
        id,
      ]);
      return rows[0];
    },
    ...
    delete: async (id) => {
      const [result] = await pool.execute("DELETE FROM skills WHERE id = ?", [
        id,
      ]);
      return result;
    },
